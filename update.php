...
if (isset($conn)) { // Check if connection was successful
    if (isset($_GET['advertiseName'])) {
        ...
        $sql = "SELECT * FROM advertise_deials WHERE advertiseName = '$advertiseName'";
        ...
        if ($result->num_rows > 0) { // Check if records exist
            ...
            echo "<form action='./insert.inc.php' method='post' id='adForm' style='display: flex; flex-direction: column; width: 300px; gap: 10px;'>";

            echo "<label for='advertiserName'>Advertiser Name</label>";
            echo "<input type='text' id='advertiserName' name='advertiserName' value='" . $advertiserName . "' required>";
            ...
            echo "<input type='text' id='adBudget' name='adBudget' value='" . $adBudget . "' required>";
